...
esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t device_addr, uint8_t *data_wr, size_t size);
esp_err_t i2c_master_read_slave(i2c_port_t i2c_num, uint8_t device_addr, uint8_t data_addr, uint8_t *regData, size_t size);
//end i2c define section
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    cmd_process((p_data->write.value),  p_data->write.len);
#endif
                }else{
                    //TODO:
                }
...
uint32_t cmdDigits[256];  //store ASCII(digits) from PC with "," as split
uint8_t tempdata;
uint8_t realInt[128];  //
uint8_t versionInfo[] =
{'1','0','0','1','\r'};//add scanner result to bluetooth notification, 9/14/2021
//{'1','0','0','0','\r'};//initial release
//history
...
int cmd_process(const void* cmd, size_t size)
{
	...
	memcpy(Buf, cmd, size);
	 ...
	 //process data
	  ...
	  //bool done = false;//if true done with find cmd string
	  //finding string
	  ...
	  //cmdDigits need convert to real int, split with , (44)
	  //process date from PC like "abc:12,33,46,89\r"
...
//start process cmd task, format using A:x,x,x....\r
	if(cmdString[0]==65)//if start with "A", can be any string base on created the own format
	{
		...
		case 2://i2c write
			...
			esp_err_t ret = i2c_master_write_slave(I2C_MASTER_NUM, deviceAddr, temp, 2);
			if(ret==ESP_OK)
			{
			...
			{
				uart_write_bytes(UART_NUM_0, "Write NG\r", 8);
			}
		...
		case 3://i2c read
			...
			uint8_t *data = malloc(reg_data_size);
			i2c_master_read_slave(I2C_MASTER_NUM, deviceAddr, regAddr, data, reg_data_size);

			esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],2, data, false);//send register data to phone (notification)

			for(int i=0;i<reg_data_size;i++)
			{
				sprintf(sTxBuffer,"data from i2c, %02X  \r", data[i]);
			...
			break;
		case 4://i2c scanner
			i2c_count = do_i2cdetect_cmd();
			esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL], i2c_count, i2c_device, false);//send register data to phone (notification)

			for (int i = 0; i < i2c_count; i++) {
				sprintf(sTxBuffer,"i2c devices, %02X  \r", i2c_device[i]);
				uart_write_bytes(UART_NUM_0, sTxBuffer, sizeof(sTxBuffer));
				//needs some delay for uart works
				vTaskDelay(50 / portTICK_PERIOD_MS);
			}
...
//i2c_set_timeout(i2c_port,200000);//default timeout is 0x1f40=8000, doesn't work with io port expander(tested with PCA9698, CY8C9540A
...
esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t device_addr, uint8_t *data_wr, size_t size)
{
	...
	i2c_set_timeout(I2C_MASTER_NUM,20000);//default timeout is 0x1f40=8000, doesn't work with io port expander(tested with PCA9698, CY8C9540A
...
esp_err_t i2c_master_read_slave(i2c_port_t i2c_num, uint8_t device_addr, uint8_t data_addr, uint8_t *regData, size_t size)
{
	...
	if (size > 1) {
		i2c_master_read(cmd, regData, size - 1, ACK_VAL);
	}
	i2c_master_read_byte(cmd, regData + size - 1, NACK_VAL);
	i2c_master_stop(cmd);
...
int do_i2cdetect_cmd()
{
    ...
    i2c_cmd_link_delete(cmd);
    printf("Found I2C Devices: %d \n", i2c_count);//have to add\n
    return i2c_count;
}
